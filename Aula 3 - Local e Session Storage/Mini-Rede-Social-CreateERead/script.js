...
function editarPost(index){
    ...
    const novaCategoria = prompt('Edite a categoria do post',posts[index].category);
    posts[index].category = novaCategoria;
    const novaImagem = prompt('Edite a imagem do post (insira o link da imagem)',posts[index].image);
    posts[index].image = novaImagem;
...
function apagarPost(index){
    const confirmar = confirm("Você deseja realmente excluir este post?")
